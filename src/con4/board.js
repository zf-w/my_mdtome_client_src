...
export function make_board_with_wrapper_elem_string(
  actions_string,
  w_num,
  h_num,
  wrapper_elem_id_string
) {
  return `<div id="${wrapper_elem_id_string}">${make_board_elem_string(
    actions_string,
    w_num,
    h_num
  )}</div>`;
...
/**
 *
 * @param {HTMLElement} elem_mut_ref
 * @param {{
 * w: number,
 * h: number,
 * actions_list: number[]
 * }} data
 */
export function render_game_board_with_actions_list(elem_mut_ref, data) {
  const { w, h, actions_list } = data;
  elem_mut_ref.innerHTML = make_board_elem_string(actions_list, w, h);
}
...
export function render_con4_board(elem_id, data) {
  ...
  elem.innerHTML = make_board_elem_string(
    con4.actions_string_to_list(actions_string),
    w,
    h
  );
}
